...
DASHBOARD_AI_SYSTEM_PROMPT = (
    ...
    "estimate a number that isn't present. Do not mention SQL, databases, or "
    "JSON in the text itself."
)
...
def _fallback_dashboard_summary(stats):
    return (
        ...
        f"{stats['utilization_pct']}% of ₹{stats['raised_amount']:,.0f} raised "
        f"utilized so far, reaching {stats['beneficiaries']:,} beneficiaries. "
...
def ai_summary():
    ...
    ai = generate_ai_json(DASHBOARD_AI_SYSTEM_PROMPT, stats)
